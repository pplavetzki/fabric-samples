...
func main() {
	conf := config.FromFile(filepath.Join("..", "..", "..", "gateway/connection-org2.yaml"))
	// Instantiate the SDK
	...
	mspClient, err := msp.New(ctx, msp.WithCAInstance("ca.org2.example.com"))
	if err != nil {
	...
	err = wallet.Put("isabella", identity)
	if err != nil {
